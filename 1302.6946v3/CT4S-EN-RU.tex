...
\newenvironment{sloganRUS}{\begin{russian}\addtocounter{envENG}{1}\vspace{.1in}\begin{sloppypar}\noindent{\em Лозунг}\;\arabic{envENG}: \begin{quote}{\bf «}\slshape}{{\bf »}\end{quote}\end{sloppypar}\vspace{.1in}\end{russian}}
...
\begin{russian}Каждый факт состоит из двух путей, допустим, $P$ и $Q$, которые мы намерены объявить эквивалентными. Пути $P$ и $Q$ обязательно должны иметь общее начало и конец, скажем, $s$ и $t$, однако их длины могут отличаться, скажем, $m$ и $n$ соответственно.
...
\begin{applicationRUS}
\begin{russian}A traditional (Mendelian) way to predict the genotype of offspring based on the genotype of its parents is by the use of \href{http://en.wikipedia.org/wiki/Punnett_square}{Punnett squares}. If $F$ is the set of possible genotypes for the female parent and $M$ is the set of possible genotypes of the male parent, then $F\times M$ is drawn as a square, called a Punnett square, in which every combination is drawn. \end{russian}
\end{applicationRUS}
...
\begin{exerciseRUS}
\begin{russian}How many elements does the set $\{a,b,c,d\}\times\{1,2,3\}$ have?\end{russian}
\end{exerciseRUS}
