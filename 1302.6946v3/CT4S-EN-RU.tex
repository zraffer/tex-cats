...
\renewcommand{\contentsname}{Contents / Soderzhanie}% problem
...
\newtheorem{theoremENG}[subsection]{\begin{english}Theorem\end{english}}
\theoremstyle{lemmaENG}
\newtheorem{lemmaENG}[subsection]{\begin{english}Lemma\end{english}}
\theoremstyle{propositionENG}
\newtheorem{propositionENG}[subsection]{\begin{english}Proposition\end{english}}
\theoremstyle{corollaryENG}
\newtheorem{corollaryENG}[subsection]{\begin{english}Corollary\end{english}}
\theoremstyle{factENG}
\newtheorem{factENG}[subsection]{\begin{english}Fact\end{english}}

\theoremstyle{remarkENG}
\newtheorem{remarkENG}[subsection]{\begin{english}Remark\end{english}}
\theoremstyle{exampleENG}
\newtheorem{exampleENG}[subsection]{\begin{english}Example\end{english}}
\theoremstyle{warningENG}
\newtheorem{warningENG}[subsection]{\begin{english}Warning\end{english}}
\theoremstyle{questionENG}
\newtheorem{questionENG}[subsection]{\begin{english}Question\end{english}}
\theoremstyle{guessENG}
\newtheorem{guessENG}[subsection]{\begin{english}Guess\end{english}}
\theoremstyle{answerENG}
\newtheorem{answerENG}[subsection]{\begin{english}Answer\end{english}}
\theoremstyle{constructionENG}
\newtheorem{constructionENG}[subsection]{\begin{english}Construction\end{english}}
\theoremstyle{rulesENG}
\newtheorem{rulesENG}[subsection]{\begin{english}Rules of good practice\end{english}}
\theoremstyle{excENG}
\newtheorem{excENG}[subsection]{\begin{english}Exercise\end{english}}
\theoremstyle{appENG}
\newtheorem{appENG}[subsection]{\begin{english}Application\end{english}}
\theoremstyle{definitionENG}
\newtheorem{definitionENG}[subsection]{\begin{english}Definition\end{english}}
\theoremstyle{notationENG}
\newtheorem{notationENG}[subsection]{\begin{english}Notation\end{english}}
\theoremstyle{conjectureENG}
\newtheorem{conjectureENG}[subsection]{\begin{english}Conjecture\end{english}}
\theoremstyle{postulateENG}
\newtheorem{postulateENG}[subsection]{\begin{english}Postulate\end{english}}
...
\newenvironment{sloganENG}{\addtocounter{subsection}{1}\vspace{.1in}\begin{sloppypar}\noindent{\em Slogan}\;\arabic{chapter}.\arabic{section}.\arabic{subsection}.\arabic{subsection}. \begin{quote}``\slshape}{"\end{quote}\end{sloppypar}\vspace{.1in}}

%%%%%%%%%%%%%%%% заголовки теорем, RUS

\theoremstyle{theoremRUS}
\newtheorem{theoremRUS}[subsection]{\begin{russian}Теорема\end{russian}}
\theoremstyle{lemmaRUS}
\newtheorem{lemmaRUS}[subsection]{\begin{russian}Лемма\end{russian}}
\theoremstyle{propositionRUS}
\newtheorem{propositionRUS}[subsection]{\begin{russian}Утверждение\end{russian}}
\theoremstyle{corollaryRUS}
\newtheorem{corollaryRUS}[subsection]{\begin{russian}Следствие\end{russian}}
\theoremstyle{factRUS}
\newtheorem{factRUS}[subsection]{\begin{russian}Факт\end{russian}}

\theoremstyle{remarkRUS}
\newtheorem{remarkRUS}[subsection]{\begin{russian}Примечание\end{russian}}
\theoremstyle{exampleRUS}
\newtheorem{exampleRUS}[subsection]{\begin{russian}Пример\end{russian}}
\theoremstyle{warningRUS}
\newtheorem{warningRUS}[subsection]{\begin{russian}Предупреждение\end{russian}}
\theoremstyle{questionRUS}
\newtheorem{questionRUS}[subsection]{\begin{russian}Вопрос\end{russian}}
\theoremstyle{guessRUS}
\newtheorem{guessRUS}[subsection]{\begin{russian}Догадка\end{russian}}
\theoremstyle{answerRUS}
\newtheorem{answerRUS}[subsection]{\begin{russian}Ответ\end{russian}}
\theoremstyle{constructionRUS}
\newtheorem{constructionRUS}[subsection]{\begin{russian}Конструкция\end{russian}}
\theoremstyle{rulesRUS}
\newtheorem{rulesRUS}[subsection]{\begin{russian}Рекомендации\end{russian}}
\theoremstyle{excRUS}
\newtheorem{excRUS}[subsection]{\begin{russian}Упражнение\end{russian}}
\theoremstyle{appRUS}
\newtheorem{appRUS}[subsection]{\begin{russian}Прикладной пример (применение)\end{russian}}
\theoremstyle{definitionRUS}
\newtheorem{definitionRUS}[subsection]{\begin{russian}Определение\end{russian}}
\theoremstyle{notationRUS}
\newtheorem{notationRUS}[subsection]{\begin{russian}Обозначение\end{russian}}
\theoremstyle{conjectureRUS}
\newtheorem{conjectureRUS}[subsection]{\begin{russian}Гипотеза\end{russian}}
\theoremstyle{postulateRUS}
\newtheorem{postulateRUS}[subsection]{\begin{russian}Постулат\end{russian}}
...
\newenvironment{sloganRUS}{\begin{russian}\addtocounter{subsection}{1}\vspace{.1in}\begin{sloppypar}\noindent{\em Лозунг}\;\arabic{chapter}.\arabic{section}.\arabic{subsection}.\arabic{subsection}. \begin{quote}«\slshape}{»\end{quote}\end{sloppypar}\vspace{.1in}\end{russian}}
...
\begin{russian}Теория множеств была изобретена [развита?] в качестве оснований всей математики. Понятия множества и функции служат основой, на которой строится наша интуиция о категориях вообще. В этой главе мы дадим примеры множеств и функций и затем перейдем к обсуждению коммутативных диаграмм. К тому моменту мы сможем ввести ологи, которые позволят нам использовать язык теории категорий, чтобы говорить о понятиях реального мира. Затем мы введем пределы и копределы и их универсальные свойства. Весь этот материал входит в область элементарной теории множеств, но он может также рассматриваться и как исследование нашей первой категории, {\em категории множеств}, которую мы назовем $\Set$. \end{russian}
...
\footnote{«Каноническая» означает нечто наподобие «лучшего выбора», выбора, который выделяется, как единственный обоснованный.}\index{canonical}
\end{russian}
\end{exerciseRUS}
...
\begin{russian} 
При печати прямоугольников с тестом внутри абзаца возникают проблемы типографского характера, например прямоугольник \fbox{человек} здесь выглядит выползающим за границы строки, и чем больше таких прямоугольников изпользуется, тем хуже они выглядят. Чтобы исправить это, я буду обозначать типы, которые появляются внутри текста, символами-уголками; например, я буду писать \fakebox{человек} вместо \fbox{человек}.
\end{russian}
