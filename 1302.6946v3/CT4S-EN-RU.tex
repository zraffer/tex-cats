...
\titlepic{\vspace{1.3in}\dashbox{\includegraphics[width=.8\textwidth]{ScientificMethod}}\\\vspace{.3in}\Large How can mathematics make this diagram meaningful? \\ \begin{russian}Как математика может сделать эту диаграмму более осмысленной? \end{russian}
\normalsize}
\maketitle
...
\next Думаете ли вы, что схемы связей между другими областями мозга функцие-подобны? 
\endsexc 
\end{russian}
...
\begin{russian}
\sexc Найти множество $A$ такое, что для всех множеств $X$ в $\Hom_\Set(X,A)$ имеется в точности один элемент. Подсказка: нарисовать картинку со всевозможными $A$ и $X$.
...
\begin{definition}[Isomorphism]\label{def:iso in set} Let $X$ and $Y$ be sets. A function $f\taking X\to Y$ is called an {\em isomorphism}\index{function!isomorphism}\index{isomorphism!of sets}, denoted $f\taking X\To{\iso}Y$, if there exists a function $g\taking Y\to X$ such that $g\circ f=\id_X$ and $f\circ g=\id_Y$. We also say that $f$ is {\em invertible} and we say that $g$ is {\em the inverse}\index{function!inverse} of $f$. If there exists an isomorphism $X\To\iso Y$ we say that $X$ and $Y$ are {\em isomorphic} sets and may write $X\iso Y$. \index{a symbol!$\iso$}\end{definition}

\begin{russian}\begin{definition}[Изоморфизм]\label{def:iso in set} Пусть $X$ и $Y$ --- множества. Функция $f\taking X\to Y$ называется {\em изоморфизмом}\index{function!isomorphism}\index{isomorphism!of sets}, что обозначается как $f\taking X\To{\iso}Y$, если существует функция $g\taking Y\to X$ такая, что $g\circ f=\id_X$ и $f\circ g=\id_Y$. Мы также говорим, что $f$ является {\em обратимой}, и что $g$ является {\em обратной}\index{function!inverse} к $f$. Если существует изоморфизм $X\To\iso Y$, то мы говорим, что $X$ и $Y$ являются {\em изоморфными} множествами, и записываем это как $X\iso Y$. \index{a symbol!$\iso$}\end{definition}\end{russian}
...
\begin{russian}Если $X$ и $Y$ --- множества и $f\taking X\to Y$ --- изоморфизм, то аналог Диаграммы \ref{dia:setmap} будет выглядеть как идеальное совпадение, чаще называемое {\em соответствием один-к-одному}\index{one-to-one correspondence}\index{correspondence!one-to-one}. Это означает, что никакие две стрелки не попадают в один элемент $Y$, и что каждый элемент $Y$ находится в образе. Например, ниже изображен изоморфизм $X\To{\iso}Y$.\end{russian}
...
\begin{russian}
\begin{definition}[Мощность конечных чисел]\label{def:cardinality}
