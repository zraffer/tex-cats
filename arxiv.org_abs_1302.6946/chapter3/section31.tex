...
{\em Моноид} — это тройка $(M,e,\star),$ где $M$ — множество, $e\in M$ — элемент, а $\star\taking M\times M\to M$ — такая функция, что следующие условия выполняются для всех $m,n,p\in M$:
\begin{itemize}
\item $m\star e=m,$
\item $e\star m=m,$
\item $(m\star n)\star p=m\star(n\star p).$
...
\begin{exerciseRUS}
Предложите операцию на множестве $M=\{1,2,3,4\},$ другими словами — такую корректную функцию $f\taking M\times M\to M,$ что $f$ не может быть умножением для моноида на $M.$ То есть такую, которая либо не была бы ассоциативной, либо ни один элемент в $M$ не был бы ее единицей.
\end{exerciseRUS}
...
\begin{exampleRUS}
Предположим, что $(M,e,\star)$ — некоторый моноид. Для заданных элементов $m_1,m_2,m_3,m_4$ имеется пять различных способов расстановки скобок в произведении $m_1\star m_2\star m_3\star m_4,$ при этом закон ассоциативности для моноидов гарантирует, что все они равны. Выполняются равенства:
\begin{align*}
((m_1\star m_2)\star m_3)\star m_4&=(m_1\star m_2)\star (m_3\star m_4)\\
&=(m_1\star(m_2\star m_3))\star m_4\\
&=m_1\star(m_2\star (m_3\star m_4))\\
...
Пусть $X$ — некоторое множество. {\em Список с элементами из $X$} — это пара $(n,f),$ где $n\in\NN$ — натуральное число (называемое {\em длиной списка}), а $f\taking\ul{n}\to X$ — функция, где $\ul{n}=\{1,2,\ldots,n\}.$ Его можно обозначить так:
$$(n,f)=[f(1),f(2),\ldots,f(n)].$$
...
Пусть $X$ — некоторое множество. {\em Свободный моноид, порожденный $X$} — это последовательность $M:=(\List(X),[\,],\plpl),$ где носитель $\List(X)$ — множество списков с элементами из $X,$ единица $[\,]\in\List(X)$ — пустой список, а $\plpl$ — операция конкатенации. Мы будем говорить об $X$ как о множестве, порождающем моноид $M,$ а о его элементах — как о порождающих.
\end{definitionRUS}
...
Пусть $G$ — некоторое конечное множество, а $n\in\NN$ — натуральное число,%
...
Пусть $(M,e,\star)$ — некоторый моноид, а $S$ — произвольное множество. {\em Действием $(M,e,\star)$ на $S,$}%
...
\item Можете ли вы представить состояние $s$ игры $G,$ и два таких различных элемента $\ell,\ell'\in\List(B),$ что $\ell\acts s=\ell'\acts s?$ Как бы вы назвали на жаргоне видеоигр такой элемент $b\in B,$ что для любого состояния $s\in G$ выполняется $b\acts s=s?$
\item Как бы вы назвали на жаргоне видеоигр такое состояние $s\in S,$ что для каждой последовательности кнопок $\ell\in\List(B)$ выполняется $\ell\acts s=s?$
\endsexc
\end{exerciseRUS}
...
Пусть $f\taking\RR\to\RR$ — это дифференцируемая функция, у которой мы хотим найти корни (такие точки $x\in\RR,$ что $f(x)=0$). Пусть $x_0\in\RR$ — начальная точка. Для любого $n\in\NN$ мы можем применить \href{https://ru.wikipedia.org/wiki/%D0%9C%D0%B5%D1%82%D0%BE%D0%B4_%D0%9D%D1%8C%D1%8E%D1%82%D0%BE%D0%BD%D0%B0}{\text метод Ньютона} к текущей точке $x_n,$ получая следующую $$x_{n+1}=x_n-\frac{f(x_n)}{f'(x_n)}.$$
...
Пусть $\Sigma, S$ — некоторые конечные непустые множества. Задание функции $\delta\taking\Sigma\times S\to S$ эквивалентно заданию действия свободного моноида $\List(\Sigma)$ на $S.$
\end{propositionRUS}
...
Если $\Sigma$ и $S$ — множества с Рисунка~\ref{fig:fsa}, то изображенное там действие $\List(\Sigma)$ на $S$ можно представить следующей таблицей:
\begin{align}\label{dia:action table for FSM}
\begin{tabular}{| l || l | l |}\bhline
...
\begin{blockRUS}
Напомним, что $(\NN,0,+)$ является свободным моноидом с одним порождающим. Оказывается, что для любого другого моноида $\mcM=(M,e,\star)$ множество гомоморфизмов моноидов $\NN\to\mcM$ изоморфно множеству $M.$ Это частный случай (в котором $G$ — это множество из одного элемента) следующего утверждения.
\end{blockRUS}
...
Пусть $G$ — некоторое множество, $F(G):=(\List(G),[\,],\plpl)$ — свободый моноид на $G,$ а $\mcM:=(M,e,\star)$ — произвольный моноид. Имеется естественный изоморфизм
$$\Hom_\Mon(F(G),\mcM)\To{\iso}\Hom_\Set(G,M).$$
\end{propositionRUS}
...
\begin{proofRUS}
Мы предложим функцию $\varphi\taking\Hom_\Mon(F(G),\mcM)\to\Hom_\Set(G,M)$ и функцию $\psi\taking\Hom_\Set(G,M)\to\Hom_\Mon(F(G),\mcM),$ а затем покажем, что они взаимно обратны. Сначала построим $\varphi.$ Для данного изоморфизма моноидов $f\taking F(G)\to\mcM$ нам нужно найти $\varphi(f)\taking G\to M.$ Для произвольного $g\in G$ положим $\varphi(f)(g):=f([g]).$

Теперь построим $\psi.$ Для заданного $p\taking G\to M$ нужно найти такую функцию $\psi(p)\taking\List(G)\to\mcM,$ что $\psi(p)$ является гомоморфизмом моноидов. Для списка $L=[g_1,\ldots,g_n]\in\List(G)$ зададим $\psi(p)(L):=p(g_1)\star\cdots\star p(g_n)\in M.$ В частности, $\psi(p)([\,])=e.$ Нетрудно заметить, что это на самом деле гомоморфизм моноидов. Также легко доказать, что $\varphi\circ\psi(p)=p$ для всех $p\in\Hom_\Set(G,M).$ Мы покажем, что $\psi\circ\varphi(f)=f$ для любых $f\in\Hom_\Mon(F(G),\mcM).$ Выберем некоторый $L=[g_1,\ldots,g_n]\in\List(G).$ Тогда
$$\psi(\varphi f)(L)=(\varphi f)(g_1)\star\cdots\star(\varphi f)(g_n)=f[g_1]\star\cdots\star f[g_n]=f([g_1,\ldots,g_n])=f(L).$$
\end{proofRUS}
...
\begin{exerciseRUS}
Пусть $G=\{a,b\},$ $\mcM:=(M,e,\star)$ — произвольный моноид, а $f\taking G\to M$ задана равенствами $f(a)=m$ и $f(b)=n,$ где $m,n\in M.$ Если $\psi\taking\Hom_\Set(G,M)\to\Hom_\Mon(F(G),\mcM)$ — это функция, упомянутая в доказательстве Утверждения~\ref{prop:free monoid}, а $L=[a,a,b,a,b],$ то чему будет равняться $\psi(f)(L)$ ?
\end{exerciseRUS}
...
\begin{blockRUS}
Гомоморфизм моноидов $f\taking M\to M'$ (см. Определение~\ref{def:monoid hom}) гарантирует, что элементы $M$ имеют осмысленную интерпретацию в $M'$; они действуют на $M'$ таким же образом, каким они делали это в исходном $M.$ Если у нас есть такой гомоморфизм $f$ и действие $\alpha\taking M'\times S\to S$ моноида $M'$ на множестве $S,$ то у нас есть способ задать действие $M$ на $S$. А именно, мы берем элемент $M,$ переводим его в $M',$ и действуем полученным элементом на $S.$ В терминах функций, мы берем композицию $\alpha$ с функцией $f\times\id_S\taking M\times S\to M'\times S,$ получая функцию, которую мы обозначим $$\Delta_f(\alpha)\taking M\times S\to S.$$ После Утверждения~\ref{prop:restriction of scalars} мы будем знать, что $\Delta_f(\alpha)$ действительно является действием моноида, и будем говорить, что оно получается {\em ограничением скаляров вдоль $f$}.\index{ограничение скаляров}
\end{blockRUS}
...
Пусть $\mcM:=(M,e,\star)$ и $\mcM':=(M',e',\star')$ — некоторые моноиды, $f\taking\mcM\to\mcM'$ — гомоморфизм моноидов, $S$ — множество; предположим также, что $\alpha\taking M'\times S\to S$ — это действие $\mcM'$ на $S.$ Тогда $\Delta_f(\alpha)\taking M\times S\to S,$ введенное выше, также является действием моноида.
\end{propositionRUS}
...
\begin{proofRUS}
Вспомним про Примечание~\ref{rmk:monoid action}; мы знаем, что $\alpha$ — это действие моноида, и хотим показать, что $\Delta_f(\alpha)$ — тоже действие моноида. Действительно, верно равенство $\Delta_f(\alpha)(e,s)=\alpha(f(e),s)=\alpha(e',s)=s,$ а также следующие равенства:
\begin{align*}
\Delta_f(\alpha)(m,\Delta_f(\alpha)(n,s))=\alpha(f(m),\alpha(f(n),s))&=\alpha(f(m)\star' f(n),s)\\
&=\alpha(f(m\star n),s)\\
&=\Delta_f(\alpha)(m\star n,s).
...
\begin{exampleRUS}
Пусть $\NN$ и $\ZZ$ обозначают аддитивные моноиды натуральных и целых чисел соответственно, а $i\taking\NN\to\ZZ$ — вложение, которое, как мы видели в Примере~\ref{ex:nat to int}, является гомоморфизмом моноидов. Имеется действие $\alpha\taking\ZZ\times\RR\to\RR$ моноида $\ZZ$ на множестве $\RR$ действительных чисел, заданное так: $\alpha(n,x)=n+x.$ Очевидно, это действие устроено точно так же, как если бы мы ограничили наши скаляры вдоль $\NN\ss\ZZ,$ разрешая прибавлять к действительным числам только натуральные. Действие $\Delta_i\alpha\taking\NN\times\RR\to\RR$ задано $(n,x)\in\NN\times\RR$ как $\Delta_i\alpha(n,x)=\alpha(i(n),x)=\alpha(n,x)=n+x,$ что и следовало ожидать.
\end{exampleRUS}
...
\begin{exampleRUS}
Предположим, что $V$ — некоторое комплексное векторное пространство. Это, в частности, означает, что моноид $\CC$ комплексных чисел (с умножением) действует на элементах $V.$ Если $i\taking\RR\to\CC$ — это вложение действительной прямой в $\CC,$ то $i$ — гомоморфизм моноидов. Ограничение скаляров в вышеописанном смысле превращает $V$ в действительное векторное пространство, так что название «ограничение скаляров» здесь вполне уместно.
\end{exampleRUS}
...
\begin{exerciseRUS}
Пусть $\NN$ — это свободный моноид с одним порождающим, $\Sigma=\{a,b\},$ и $S=\{\tn{State 0, State 1, State 2}\}.$ Рассмотрим отображение моноидов $f\taking\NN\to\List(\Sigma),$ заданное так: $1\mapsto [a,b,b].$ Действие моноида $\alpha\taking\List(\Sigma)\times S\to S,$ определенное в Примере~\ref{ex:action table} можем быть преобразовано ограничением скаляров вдоль $f$ в действие $\Delta_f(\alpha)$ моноида $\NN$ на $S.$ Запишите таблицу этого действия.
\end{exerciseRUS}
