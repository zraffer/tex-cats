...
Пусть $G$ — это множество, $F(G):=(\List(G),[\,],\plpl)$ — свободый моноид на $G,$ а $\mcM:=(M,e,\star)$ — произвольный моноид. Имеется естественный изоморфизм
$$\Hom_\Mon(F(G),\mcM)\To{\iso}\Hom_\Set(G,M).$$
\end{propositionRUS}
...
Пусть $\mcM:=(M,e,\star)$ и $\mcM':=(M',e',\star')$ — это моноиды, $f\taking\mcM\to\mcM'$ — гомоморфизм моноидов, $S$ — множество; предположим также, что $\alpha\taking M'\times S\to S$ — это действие $\mcM'$ на $S.$ Тогда $\Delta_f(\alpha)\taking M\times S\to S,$ введенное выше, также является действием моноида.
\end{propositionRUS}
