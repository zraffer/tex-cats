...
Пусть $S$ — некоторое множество, а $R\ss S\times S$ — бинарное отношение на $S$; при этом, если $(s,s')\in R,$ то этот факт мы будем обозначать так: $s\leq s'.$ Мы называем $R$ {\em предпорядком}\index{порядок!предпорядок}, если для всех $s,s',s''\in S$ выполняются следующие свойства:
\begin{description}
