...
\begin{remarkRUS}
Если подходить к вопросу более педантично, условия из Определения \ref{def:monoid} следовало бы сформулировать так:
\begin{itemize}
\item $\star(m,e)=m$,
\item $\star(e,m)=m$,
\item $\star(\star(m,n),p)=\star(m,(\star(n,p))$.
\end{itemize}
Способ обозначения, использованный в Определении \ref{def:monoid} называется {\em инфиксной нотацией},\index{infix notation} and we often use infix notation without mentioning it. That is, given a function $\cdot\taking A\times B\to C$, we may write $a\cdot b$ rather than $\cdot(a,b)$.
\end{remarkRUS}
...
\begin{exampleRUS}[Additive monoid of natural numbers]\label{ex:monoid 0}\index{monoid!additive natural numbers}
\end{exampleRUS}
...
\begin{exampleRUS}[Trivial monoid]\label{ex:trivial monoid}
\end{exampleRUS}
...
\begin{definitionRUS}[Presented monoid]\label{def:presented monoid}\index{monoid!presented}
\end{definitionRUS}
...
\subsubsection{\caseENGRUS{Finite state machines}{ / }{Конечные автоматы}}\label{sec:FSMs}\index{finite state machine}
...
\subsection{\caseENGRUS{Monoid action tables}{ / }{Таблицы действия моноидов}}\label{sec:monoid action table}\index{action table}
...
\begin{exampleENG}[Action table]\label{ex:action table}
...
\begin{exampleRUS}[Action table]\label{ex:action table}
\end{exampleRUS}
