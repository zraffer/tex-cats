...
\item $\star(\star(m,n),p)=\star(m,(\star(n,p))$.
\end{itemize}
Способ обозначения, использованный в Определении \ref{def:monoid} называется {\em инфиксной нотацией},\index{инфиксная нотация}, и он часто используется без дополнительных предупреждений. В частности, для заданной функции $\cdot\taking A\times B\to C$ мы пишем $a\cdot b$ вместо $\cdot(a,b)$.
\end{remarkRUS}
