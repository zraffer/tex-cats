...
\item $\star(\star(m,n),p)=\star(m,(\star(n,p)).$
\end{itemize}
Однако в Определении~\ref{def:monoid} использован способ обозначения под названием {\em инфиксная нотация}\index{инфиксная нотация}, и он часто будет появляться без дополнительных предупреждений. В частности, для заданной функции $\cdot\taking A\times B\to C$ мы пишем $a\cdot b$ вместо $\cdot(a,b).$
\end{remarkRUS}
