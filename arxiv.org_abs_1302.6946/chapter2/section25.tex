...
\fbox{\xymatrixnocompile{&\obox{C}{.7in}{\raggedright a loyal customer}\LA{d}{is}\\\obox{B}{.7in}{\raggedright a wealthy customer}\LA{r}{is}&\smbox{D}{a customer}}}\hsp&\fbox{\xymatrix{\obox{A=B\times_DC}{.9in}{\raggedright a customer that is wealthy and loyal}\LAL{d}{is}\LA{r}{is}&\obox{C}{.7in}{\raggedright a loyal customer}\LA{d}{is}\\\obox{B}{.7in}{\raggedright a wealthy customer}\LA{r}{is}&\smbox{D}{a customer}}}
\end{align}
...
\begin{align}\label{dia:wealthy and loyal}
\fbox{\xymatrixnocompile{&\obox{C}{.7in}{\raggedright лояльный потребитель}\LA{d}{это}\\\obox{B}{.7in}{\raggedright богатый потребитель}\LA{r}{это}&\smbox{D}{потребитель}}}\hsp&\fbox{\xymatrix{\obox{A=B\times_DC}{.9in}{\raggedright богатый и лояльный потребитель}\LAL{d}{это}\LA{r}{это}&\obox{C}{.7in}{\raggedright лояльный потребитель}\LA{d}{это}\\\obox{B}{.7in}{\raggedright богатый потребитель}\LA{r}{это}&\smbox{D}{потребитель}}}
\end{align}
...
Заметим, что в Диаграмме (\ref{dia:wealthy and loyal}) верхний левый прямоугольник может быть (неканонически) назван \fakebox{хороший потребитель}. Если указать именно его в качестве пулбека, то автор олога будет фактически {\em определять} хорошего потребителя как того, кто одновременно богат и лоялен. 
\end{remarkRUS}
...
\sexc\begin{align*}\scriptsize\fbox{\xymatrix{&&\smbox{C}{blue}\LA{d}{is}\\\smbox{B}{a person}\LA{rr}{\parbox{.7in}{\raggedright has as favorite color}}&&\smbox{D}{a color}}}\hsp&\scriptsize\fbox{\xymatrixnocompile{\obox{A=B\times_DC}{1.1in}{\raggedright a person whose favorite color is blue}\LAL{d}{is}\LA{rr}{\parbox{.7in}{\raggedright has as favorite color}}&&\smbox{C}{blue}\LA{d}{is}\\\smbox{B}{a person}\LA{rr}{\parbox{.7in}{\raggedright has as favorite color}}&&\smbox{D}{a color}}}
\end{align*}
\item\begin{align*}\scriptsize\fbox{\xymatrixnocompile{&&\smbox{C}{a woman}\LA{d}{is}\\\smbox{B}{a dog}\LA{rr}{\parbox{.7in}{\raggedright has as owner}}&&\smbox{D}{a person}}}\hsp&\scriptsize\fbox{\xymatrixnocompile{\obox{A=B\times_DC}{1in}{\raggedright a dog whose owner is a woman}\LAL{d}{is}\LA{rr}{\parbox{.7in}{\raggedright has as owner}}&&\smbox{C}{a woman}\LA{d}{is}\\\smbox{B}{a dog}\LA{rr}{\parbox{.7in}{\raggedright has as owner}}&&\smbox{D}{a person}}}
\end{align*}
\item\begin{align*}\scriptsize\fbox{\xymatrixnocompile{&\obox{C}{.5in}{\raggedright a piece of furniture}\LA{d}{has}\\\obox{B}{.6in}{\raggedright a space in our house}\LA{r}{has}&\smbox{D}{a width}}}\hsp&\scriptsize\fbox{\xymatrixnocompile{\obox{A=B\times_DC}{.5in}{\raggedright a good fit}\LAL{d}{$s$}\LA{r}{$f$}&\obox{C}{.5in}{\raggedright a piece of furniture}\LA{d}{has}\\\obox{B}{.6in}{\raggedright a space in our house}\LA{r}{has}&\smbox{D}{a width}}}
\end{align*}
\endsexc
\end{exerciseENG}
...
\sexc\begin{align*}\scriptsize\fbox{\xymatrix{&&\smbox{C}{синий}\LA{d}{это}\\\smbox{B}{человек}\LA{rr}{\parbox{.7in}{\raggedright любимый цвет}}&&\smbox{D}{цвет}}}\hsp&\scriptsize\fbox{\xymatrixnocompile{\obox{A=B\times_DC}{1.1in}{\raggedright человек, который любит синий цвет}\LAL{d}{это}\LA{rr}{\parbox{.7in}{\raggedright любимый цвет}}&&\smbox{C}{синий}\LA{d}{это}\\\smbox{B}{человек}\LA{rr}{\parbox{.7in}{\raggedright любимый цвет}}&&\smbox{D}{цвет}}}
\end{align*}
\item\begin{align*}\scriptsize\fbox{\xymatrixnocompile{&&\smbox{C}{женщина}\LA{d}{это}\\\smbox{B}{собака}\LA{rr}{\parbox{.7in}{\raggedright владелец}}&&\smbox{D}{человек}}}\hsp&\scriptsize\fbox{\xymatrixnocompile{\obox{A=B\times_DC}{1in}{\raggedright собака, у которой владелец — женщина}\LAL{d}{это}\LA{rr}{\parbox{.7in}{\raggedright владелец}}&&\smbox{C}{женщина}\LA{d}{это}\\\smbox{B}{собака}\LA{rr}{\parbox{.7in}{\raggedright владелец}}&&\smbox{D}{человек}}}
\end{align*}
\item\begin{align*}\scriptsize\fbox{\xymatrixnocompile{&\obox{C}{.5in}{\raggedright предмет мебели}\LA{d}{имеет}\\\obox{B}{.6in}{\raggedright свободное место в доме}\LA{r}{имеет}&\smbox{D}{ширина}}}\hsp&\scriptsize\fbox{\xymatrixnocompile{\obox{A=B\times_DC}{.5in}{\raggedright подходит}\LAL{d}{$s$}\LA{r}{$f$}&\obox{C}{.5in}{\raggedright предмет мебели}\LA{d}{имеет}\\\obox{B}{.6in}{\raggedright свободное место в доме}\LA{r}{имеет}&\smbox{D}{ширина}}}
\end{align*}
\endsexc
\end{exerciseRUS}
...
Let $f\colon X{→} Y$ be a function and $y\in Y$ an element. The {\em preimage of y under $f$}\index{preimage}, denoted $f^{-1}(y),$\index{a symbol!$f^{-1}$} is  the subset $f^{-1}(y){\coloneqq}\{x\in X{\;|\;}f(x)=y\}.$ If $Y'\subseteq Y$ is any subset, the {\em preimage of $Y'$ under $f$}, denoted $f^{-1}(Y'),$ is the subset $f^{-1}(Y')=\{x\in X{\;|\;}f(x)\in Y'\}.$
\end{definitionENG}

\begin{definitionRUS}[Прообраз]\label{def:preimage}
Пусть $f\colon X{→} Y$ — некоторая функция и $y\in Y$ — элемент ее области значений. {\em Прообраз $y$ относительно $f$}\index{прообраз}, обозначаемый $f^{-1}(y),$\index{символ!$f^{-1}$} — это подмножество $f^{-1}(y){\coloneqq}\{x\in X{\;|\;}f(x)=y\}.$ Если $Y'\subseteq Y$ — произвольное подмножество, то {\em прообраз $Y'$ относительно $f$}, обозначаемый $f^{-1}(Y'),$ — это подмножество $f^{-1}(Y')=\{x\in X{\;|\;}f(x)\in Y'\}.$
\end{definitionRUS}
...
\small
\begin{align}\label{dia:bad battery}
\fbox{\xymatrixnocompile{\obox{A\iso B\times_DC}{1in}{мобильный телефон с плохим аккумулятором}\ar[r]\ar[d]&\smbox{C\iso D\times_FE}{плохой аккумулятор}\ar[r]\ar[d]&\obox{E\iso F\times_HG}{.5in}{менее 1 часа}\ar[r]\ar[d]&\obox{G}{.5in}{между 0 и 1}\ar[d]\\\smbox{B}{мобильный телефон}\LA{r}{имеет}&\smbox{D}{аккумулятор}\LA{r}{\parbox{.4in}{\raggedright остается заряженным}}&\obox{F}{.6in}{промежуток времени}\LA{r}{\hspace{.07in}\parbox{.4in}{\raggedright в часах}}&\obox{H}{.6in}{числовой промежуток}}}
\end{align}\normalsize
\end{blockRUS}
...
\begin{exerciseRUS}~
\sexc Придумать олог, который при помощи пулбека определяет, что два человека «имеют приблизительно однаковый рост», если и только если разность их роста меньше половины дюйма. Ваш олог может включать прямоугольник \fakebox{такое действительное число $x,$ что $-.5<x<.5$}. 
