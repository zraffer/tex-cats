...
Пусть $f\taking X\to Y$ — некоторая функция и $y\in Y$ — элемент ее области значений. {\em Прообраз $y$ относительно $f$}\index{прообраз}, обозначаемый $f^\m1(y),$\index{символ!$f^\m1$} — это подмножество $f^\m1(y):=\{x\in X\|f(x)=y\}.$ Если $Y'\ss Y$ — произвольное подмножество, то {\em прообраз $Y'$ относительно $f$}, обозначаемый $f^\m1(Y'),$ — это подмножество $f^\m1(Y')=\{x\in X\|f(x)\in Y'\}.$
\end{definitionRUS}
...
\begin{exerciseRUS}
Пусть $f\taking X\to Y$ — некоторая функция и $y\in Y$ — элемент ее области значений. Нарисуйте притягивающий квадрат, в котором расслоенное произведение изоморфно прообразу $f^\m1(y).$ 
\end{exerciseRUS}
...
X\ar[r]_t&Z}
\end{align*}
Для любого множества $A$ и коммутативной диаграммы из непрерывных стрелок, данной ниже (т.е. таких функций $f\taking A\to X$ и $g\taking A\to Y,$ что $t\circ f=u\circ g$), 
\begin{align}\label{dia:universal property of fp}
\xymatrix{
&X\times_ZY\ar@/_1pc/[lddd]_{\pi_1}\ar@/^1pc/[rddd]^{\pi_2}\\\\
&A\ar@{-->}[uu]^{\exists!}\ar[dl]_{\forall f}\ar[dr]^{\forall g}&\\
...
$$
Положив $W=X\times_ZY$ и $W'=X'\times_{Z'}Y',$ образуем диаграмму справа. Используя универсальное свойство расслоенных произведений, сконструируйте такое отображение $W\to W',$ чтобы все квадраты коммутировали.
\end{exerciseRUS}
...
\begin{blockRUS}
Пусть $A$ и $B$ — некоторые множества, а $A\from R\to B$ — развилка между ними. Согласно универсальному свойству произведений у нас имеется единственное отображение $R\To{p}A\times B.$ 
\end{blockRUS}
...
\sexc Терминальное множество — это такое множество $S,$ что для любого множества $X$ существует единственная функция $X\to S.$ Найдите хотя бы одно терминальное множество. 
