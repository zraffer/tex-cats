...
\subsection{Arithmetic of sets / Арифметика множеств}\label{sec:arithmetic of sets}\index{set!arithmetic of}\index{множество!арифметика}
...
\includegraphics[height=3in]{OlogNetwork5}
\end{center} 
В данном случае $X_1$ состоит из элементов вида $\{A,B\}$ и $\{D,K\}$, но не $\{D,J\}$. 

Выпишите $X_2$ и $X_3$ (подсказка: чертеж $X$ показывает, $X_3$ будет иметь один элемент).
\end{exerciseRUS}
...
\begin{exerciseRUS}
Стандартный 2-симплекс $\Delta^2$ изображается сплошным треугольником с вершинами $V=\{1,2,3\}$. Имеется симплициальный комплекс $X=\partial\Delta^2$, который можно изобразить как пустой треугольник с тем же множеством вершин. 
