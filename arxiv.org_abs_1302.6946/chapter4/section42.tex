...
\begin{blockENG}
A group is just a monoid $(M,e,\star)$ in which every element $m\in M$ is invertible, meaning there exists some $m'\in M$ with $m\star m'=e=m'\star m.$ If a monoid is the same thing as a category $\mcM$ with one object\index{group!as category}, then a group must be a category with one object and with an additional property having to do with invertibility. The elements of $M$ are the morphisms of the category $\mcM,$ so we need a notion of invertibility for morphisms. Luckily we have such a notion already, namely isomorphism. We have the following:
\end{blockENG}
...
\begin{blockENG}
A preorder $(X,\leq)$ consists of a set $X$ and a binary relation $\leq$ that is reflexive and transitive. We can make from $(X,\leq)\in\Ob(\PrO)$ a category $\mcX\in\Ob(\Cat)$ as follows. Define $\Ob(\mcX)=X$ and for every two objects $x,y\in X$ define\index{preorder!as category}
...
\begin{blockENG}
Let $\mcC$ denote the category depicted below\index{graph!as functor} 
...
\subsubsection{\caseENGRUS{Instances on a schema}{ / }{Экземпляры схемы} \texorpdfstring{$\mcC$}{C}}\label{sec:instances}
