...
Let $\mcC$ be a schema (or category). The {\em category of instances on $\mcC$}\index{a category!$\mcC\set$}\index{database!category of instances on}, denoted $\mcC\set,$ is $\Fun(\mcC,\Set).$ Its objects are $\mcC$-instances (i.e. functors $\mcC\to\Set)$ and its morphisms are natural transformations.
\end{definitionENG}
